...
import { filterRowsByName, projectTableRows, sortRows } from '@/domain/table'
...
describe('filterRowsByName', () => {
  const rows = rowsOf(
    orgNode('eng', 'Инженерия'),
    orgNode('web', 'Web', { parentId: 'eng' }),
  )
...
describe('sortRows', () => {
  const rows = rowsOf(
    ...
    orgNode('a', 'Альфа', { parentId: 'b', headcount: 8, budget: 80, performance: 90 }),
  )
