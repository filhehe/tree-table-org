...
import { parseOrgPatch, parseOrgTree } from '@/domain/schema';
...
describe('parseOrgTree — контракт API', () => {
  ...
  it('принимает ISO-8601 с Z и с числовым смещением', () => {
    ...
    expect(parseOrgTree([{ ...valid, updatedAt: '2026-09-15T16:00:00+04:00' }])[0]?.updatedAt).toBe(
  ...
  it('отклоняет не-ISO updatedAt', () => {
    ...
    expect(() => parseOrgTree([{ ...valid, updatedAt: '2026-09-15 12:00:00.000Z' }])).toThrow(
      /Невалидный ответ API/,
    );
  });
  ...
  it('строка-число в headcount — ошибка схемы, не coerce', () => {
    expect(() => parseOrgTree([{ ...valid, headcount: '3' as unknown as number }])).toThrow(
      /Невалидный ответ API/,
    );
  });
});

describe('parseOrgPatch', () => {
  it('принимает частичное обновление и отбрасывает лишние поля', () => {
    expect(
      parseOrgPatch({
        id: 'team-12',
        headcount: 8,
  ...
  it('отклоняет патч без метрик и с неизвестным JSON', () => {
    expect(parseOrgPatch({ id: 'a', updatedAt: '2026-09-15T08:00:00.000Z' })).toBeNull();
    expect(parseOrgPatch('{nope')).toBeNull();
  });
});
